...
describe('MapMaster Integration Tests - Stage-1 Fractions', () => {
  ...
  let invariantRegistry: DefaultInvariantRegistryAdapter;
  ...
  beforeEach(() => {
    ...
    invariantRegistry = new DefaultInvariantRegistryAdapter(undefined, astHelpers);
  ...
  describe('Multiple Matching Rules', () => {
    it('handles registry with multiple matching rules', () => {
      const expr = makeTestExpression_1over7_plus_3over7();
      const request = makeTestRequestForRootClick(expr);
      ...
      // Should have at least one candidate from the registry
      expect(fracCandidates.length).toBeGreaterThanOrEqual(1);
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('handles empty invariant rules gracefully', () => {
      // Use empty registry adapter
      const emptyRegistry = new (class implements any {
        getInvariantRulesForRequest() { return []; }
      })();

      const emptyRuleProvider = new DefaultMapMasterRuleProvider(
        selectionNormalizer,
        astHelpers,
        emptyRegistry,
        new NoopMapMasterLogger()
      ...
      const candidates = emptyRuleProvider.buildCandidates(request);
