 ...
 * 
 * This is a simplified adapter that uses the same underlying registry
 * as DefaultInvariantRegistryAdapter but allows adding custom rules
 * for specific test scenarios.
 */
export class TestInvariantRegistryAdapter implements InvariantRegistryAdapter {
  private customRules: InvariantRule[] = [];

  constructor(
    private readonly baseAdapter?: InvariantRegistryAdapter,
    customRules: InvariantRule[] = []
  ) {
  ...
  getInvariantRulesForRequest(
    ...
    // Get rules from base adapter if available
    const baseRules = this.baseAdapter 
      ? this.baseAdapter.getInvariantRulesForRequest(request, windowRootPath, windowRootNode)
      : [];
