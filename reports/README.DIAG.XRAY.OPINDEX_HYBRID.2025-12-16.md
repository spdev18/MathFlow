...
## 2025-12-16 Hotfix: Keep operatorIndex alongside selectionPath
If Stable-ID (selectionPath) is present but imperfect for operator clicks (e.g., `+` in some fraction layouts),
the backend may return `no-candidates` and Step1 (`*1` insertion) won't happen.
This patch keeps `operatorIndex` in the V5 payload **even when** `selectionPath` exists, so the backend can match either way.
