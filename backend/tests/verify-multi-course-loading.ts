...
import { runOrchestratorStep, OrchestratorStepRequest } from "../src/orchestrator/step.orchestrator";
import { SessionService } from "../src/session/session.service";
import { createDefaultStudentPolicy } from "../src/stepmaster/stepmaster.policy";
import type { InvariantModelDefinition } from "../src/invariants/invariants.model";

const TEMP_DIR = join(process.cwd(), "tests", "temp-courses");

function createCourseFile(filename: string, setId: string, ruleTitle: string) {
...
async function runTest() {
    ...
    try {
        ...
        // Test Course A
        const reqA: OrchestratorStepRequest = {
            sessionId: "session-a",
            courseId: "course.a",
            expressionLatex: "3 + 5",
            selectionPath: null
        ...
        // Test Course B
        const reqB: OrchestratorStepRequest = {
            sessionId: "session-b",
            courseId: "course.b",
            expressionLatex: "3 + 5",
            selectionPath: null
        ...
        // Test Invalid Course
        const reqInvalid: OrchestratorStepRequest = {
