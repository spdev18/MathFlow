...
function makeValidRule(id: InvariantRuleId, primitiveId: PrimitiveId): InvariantRuleDefinition {
  return {
    id,
    title: `Rule ${id}`,
...
describe("validateInvariantModel", () => {
  it("accepts a valid model and returns normalized copies", () => {
    ...
    expect(normalized.invariantSets[0].rules[0]).not.toBe(
      model.invariantSets[0].rules[0],
    );
  ...
  it("rejects non-object input", () => {
    const result = validateInvariantModel(null as unknown);
  ...
  it("detects duplicate primitive ids", () => {
    ...
    const duplicatePrimitive: PrimitiveDefinition = {
      ...model.primitives[0],
    };
    ...
    expect(result.ok).toBe(false);
    expect(result.issues.some((i) => i.code === "DUPLICATE_PRIMITIVE_ID")).toBe(
      true,
    );
  });
  ...
  it("detects unknown primitive ids in rules", () => {
    ...
    model.invariantSets[0].rules[0].primitiveIds = ["UNKNOWN" as PrimitiveId];

    const result = validateInvariantModel(model);

    expect(result.ok).toBe(false);
    expect(result.issues.some((i) => i.code === "UNKNOWN_PRIMITIVE_ID")).toBe(
      true,
    );
  });
  ...
  it("validates rule level against VALID_LEVELS", () => {
    ...
    (model.invariantSets[0].rules[0] as any).level = "invalid-level";

    const result = validateInvariantModel(model);

    expect(result.ok).toBe(false);
    expect(result.issues.some((i) => i.code === "INVALID_RULE_LEVEL")).toBe(
      true,
    );
    ...
    for (const level of VALID_LEVELS) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (m.invariantSets[0].rules[0] as any).level = level;
