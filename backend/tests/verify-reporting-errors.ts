...
async function runTest() {
    ...
    if (teacherReg.status === "error") throw new Error(`Teacher reg failed: ${teacherReg.error}`);
    if (studentReg.status === "error") throw new Error(`Student reg failed: ${studentReg.error}`);
    ...
    // 4. Verify Report
    console.log("\n--- Verifying Report ---");
    const studentId = (studentReg as any).userId;
    if (!studentId) throw new Error("Student ID not found in registration response");
